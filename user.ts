...
class User {
  ...
  public returnSpecificUserData(specificKey) {
    let userData = this.getUserDataFromDatabase();
  ...
  public getAllUsernames() {
    const myDatabase = this.db();
    const allUsers = myDatabase.query("SELECT * FROM user");
    const usernames = [];
